...
1. `<ShowDetails>` takes a `show` as a prop. [X]
2. `<ShowDetails>` keeps the selected episode in a state variable. [X]
3. If no show is provided, `<ShowDetails>` renders a message encouraging the user to select a show. [X]
